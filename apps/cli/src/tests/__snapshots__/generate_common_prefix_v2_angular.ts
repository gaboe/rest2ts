...
export class ApiService {
  ...
  postCountryCodeVerifyV2(body: string): Observable<ResponseResult<void, 200>> {
    const requestData = getApiRequestData<string>(body, false);

    return apiPost<ResponseResult<void, 200>>(this.httpClient, `${this.baseUrl}/v2/{countryCode}/verify`, requestData);
  }

  postCountryCodeSessionsV2(countryCode?: string, lang?: string): Observable<ResponseResult<Session, 201>> {
		const queryParams = {
      "lang": lang
    };
    const requestData = getApiRequestData<object>(undefined, false);

    return apiPost<ResponseResult<Session, 201>>(this.httpClient, `${this.baseUrl}/v2/${countryCode}/Sessions`, requestData, queryParams);
  }
}
