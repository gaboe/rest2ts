...
export class ApiService {
  ...
  getVersion(): Observable<ResponseResult<VersionResponse, 200>> {
    return apiGet<ResponseResult<VersionResponse, 200>>(this.httpClient, `${this.baseUrl}/api/version`);
  }

  postConfig(requestContract: ConfigRequest): Observable<ResponseResult<ConfigResponse, 200>> {
    const requestData = getApiRequestData<ConfigRequest>(requestContract, false);

    return apiPost<ResponseResult<ConfigResponse, 200>>(this.httpClient, `${this.baseUrl}/api/config`, requestData);
  }
}
