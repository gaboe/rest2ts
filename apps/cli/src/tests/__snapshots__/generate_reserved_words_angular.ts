...
export class ApiService {
  ...
  getEntityFor($for: string, lang?: string): Observable<ResponseResult<string, 200>> {
    const queryParams = {
      "lang": lang
    };
    return apiGet<ResponseResult<string, 200>>(this.httpClient, `${this.baseUrl}/Entity/${$for}`, queryParams);
  }

  putSessionsArguments(requestContract: UpdateSessionModel, $arguments: string, lang?: string): Observable<ResponseResult<Session, 200>> {
		const queryParams = {
      "lang": lang
    };
    const requestData = getApiRequestData<UpdateSessionModel>(requestContract, false);

    return apiPut<ResponseResult<Session, 200>>(this.httpClient, `${this.baseUrl}/Sessions/${$arguments}`, requestData, queryParams);
  }
}
