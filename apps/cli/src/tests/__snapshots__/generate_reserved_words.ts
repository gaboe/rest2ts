...
export const getEntityFor = ($for: string, lang?: string, headers = new Headers()):
  Promise<GetEntityForFetchResponse> => {
    const queryParams = {
      "lang": lang
    }
    return apiGet(`${getApiUrl()}${getEntityForPath($for)}`, headers, queryParams) as Promise<GetEntityForFetchResponse>;
}
...
export const putSessionsArguments = (requestContract: UpdateSessionModel, $arguments: string, lang?: string, headers = new Headers()):
  Promise<PutSessionsArgumentsFetchResponse> => {
    ...
    return apiPut(`${getApiUrl()}${putSessionsArgumentsPath($arguments)}`, requestData, headers, queryParams) as Promise<PutSessionsArgumentsFetchResponse>;
}
