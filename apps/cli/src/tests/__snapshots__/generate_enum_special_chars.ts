...
export type GetVersionFetchResponse = 
| FetchResponse<VersionResponse, 200> 
| ErrorResponse;

export const getVersionPath = () => `/api/version`;

export const getVersion = (options?: FetchArgsOptions):
  Promise<GetVersionFetchResponse> => {
    return apiGet(`${getApiUrl()}${getVersionPath()}`, options, {}) as Promise<GetVersionFetchResponse>;
}

export type PostConfigFetchResponse = 
| FetchResponse<ConfigResponse, 200> 
| ErrorResponse;

export const postConfigPath = () => `/api/config`;

export const postConfig = (requestContract: ConfigRequest, options?: FetchArgsOptions):
  Promise<PostConfigFetchResponse> => {
    const requestData = getApiRequestData<ConfigRequest>(requestContract, false);

    return apiPost(`${getApiUrl()}${postConfigPath()}`, requestData, options) as Promise<PostConfigFetchResponse>;
}
